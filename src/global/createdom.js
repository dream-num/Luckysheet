...
export default function luckysheetcreatedom(colwidth, rowheight, data, menu, title) {
    ...
    let flowstr = replaceHtml('<div id="luckysheetcoltable_0" class="luckysheet-cell-flow-col"> <div id ="luckysheet-sheettable_0" class="luckysheet-cell-sheettable" style="height:${height}px;width:${width}px;"></div><div id="luckysheet-bottom-controll-row" class="luckysheet-bottom-controll-row"> '+ bottomControll +' </div> </div>', { "height": escapeHtml(Store.rh_height), "width": Store.ch_width - 1 });

    let colsheader = replaceHtml(columnHeaderHTML, { "width": escapeHtml(Store.ch_width), "index": 0, "column": "" });

    flowHTML = replaceHtml(flowHTML, { "width": escapeHtml(Store.ch_width), "flow": flowstr, "index": 0 });

    gh = replaceHtml(gh, { "flow": flowHTML, "rowHeader": `<div style='height:${escapeHtml(Store.rh_height)}px' id='luckysheetrowHeader_0' class='luckysheetsheetchange'></div>`, "columnHeader": colsheader, "functionButton": luckysheetConfigsetting.functionButton });//设置需要显示的菜单
