	...
	for (let c = c1; c <= c2; c++) {
		if (filterObj == null || filterObj[c - c1] == null) {
			...
			optionHTML +=
				...
				"px;top:" +
				row_pre +
				'px;display:block;"><i class="fa fa-filter luckysheet-mousedown-cancel" aria-hidden="true"></i></div>';
		}
	}
...
function initialFilterHandler() {
	...
	//筛选按钮点击事件
	$("#luckysheet-cell-main").on("click", ".luckysheet-filter-options", function(
		e
	) {
		if (!checkProtectionAuthorityNormal(Store.currentSheetIndex, "filter")) {
			return;
