...
function labelFilterOptionState($top, optionstate, rowhidden, caljs, notSave, str, edr, cindex, stc, edc) {
    if (optionstate) {
        $top.addClass("luckysheet-filter-options-active")
            .data("rowhidden", JSON.stringify(rowhidden))
            .data("caljs", JSON.stringify(caljs))
            .html('<i class="fa fa-filter luckysheet-mousedown-cancel" aria-hidden="true"></i>');
...
function initialFilterHandler() {
    ...
    $("#luckysheet-filter-submenu").mouseover(function () {
        ...
        $("#luckysheet-filter-selected span").text($(this).find(".luckysheet-cols-menuitem-content").text()).data("value", $(this).data("value"));
            ...
            if (JSON.stringify(vmap).length > 2) {
                ...
                for (let i = 0; i < vmapKeys.length; i++) {
                    ...
                    for (let x in vmap[v]) {
                        ...
                        dataHtml = `<div class="textBox luckysheet-mousedown-cancel cf" data-check="${!contains}" 
