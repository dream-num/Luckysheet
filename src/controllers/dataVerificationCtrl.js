...
const dataVerificationCtrl = {
    ...
    rangeDialog: function (dataSource, txt) {
        ...
        $("body").first().append(replaceHtml(modelHTML, {
            ...
            "content": `<input readonly="readonly" placeholder="${dvText.selectCellRange2}" value="${escapeHtml(txt)}"/>`,
            "botton": `<button id="luckysheet-dataVerificationRange-dialog-confirm" class="btn btn-primary" data-source="${escapeHtml(dataSource)}">${buttonText.confirm}</button>
    ...
    dropdownListShow: function () {
        ...
        if (item.type === 'dropdown' && item.type2) {
            ...
            list.forEach(i => {
                ...
                optionHtml += `<div class="dropdown-List-item  luckysheet-mousedown-cancel multi${checked ? ' checked' : ''}">${escapeHtml(i)}</div>`;
            });
        } else {
            list.forEach(i => {
                optionHtml += `<div class="dropdown-List-item luckysheet-mousedown-cancel">${escapeHtml(i)}</div>`;
            });
        }
