		...
		if (_this.initialVertical) {
			...
			$("#luckysheet-freezebar-vertical")
				...
				.hover(
					function() {
						$(this)
							.parent()
							.addClass("luckysheet-freezebar-hover");
					},
					function() {
						$(this)
							.parent()
							.removeClass("luckysheet-freezebar-hover");
					}
				);
			...
			$("#luckysheet-freezebar-vertical")
				...
				.mousedown(function() {
					...
					$(this)
						.parent()
						.addClass("luckysheet-freezebar-active");
					$("#luckysheet-freezebar-vertical")
	...
	scrollAdaptOffilteroptions: function() {
		...
		if (
			...
			$(
				...
				let col_index = $(e).data("cindex"); //第几个筛选下拉框
				if (row_index >= freezen_rowindex && col_index >= freezen_colindex) {
					if (
						top < freezen_top ||
					...
					// 冻结的最后一个left值
					let frezzenEndIndexLeft = parseInt(
						$(
							"#luckysheet-filter-options-sheet" +
								Store.currentSheetIndex +
								" .luckysheet-filter-options"
						)[freezen_colindex - 1].style.left
					);
					// 当最后一个的left小于冻结的left时，代表应该隐藏了
					if (curIndexLeft < frezzenEndIndexLeft) {
						$(e).hide();
					} else {
						$(e)
							.show()
					...
					let left = Store.visibledatacolumn[col_index] - 20;
					let scrollLeft = $("#luckysheet-cell-main").scrollLeft();
					// Store.visibledatacolumn,这里存储着所有能见列的列宽
					let moveDuration = 0;
					// 动态计算被冻结的筛选框、保证随着滚动条移动不滚动
					if (scrollLeft < (freezen_colindex - 1) * 100) {
						moveDuration = left + scrollLeft;
						$(e)
							.show()
							.css({
								left: moveDuration,
