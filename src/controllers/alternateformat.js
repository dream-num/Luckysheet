...
const alternateformat = {
        ...
        if (modelCustom != null && modelCustom.length > 0) {
            ...
            for (let i = 0; i < modelCustom.length; i++) {
                ...
                modelCustomHtml += '<div class="modelbox">' +
                    '<div class="box">' +
                    '<span style="color:' + escapeHtml(color1["fc"]) + ';background-color:' + escapeHtml(color1["bc"]) + '"> — </span>' +
                    '<span style="color:' + escapeHtml(color2["fc"]) + ';background-color:' + escapeHtml(color2["bc"]) + '"> — </span>' +
                    '<span style="color:' + escapeHtml(color3["fc"]) + ';background-color:' + escapeHtml(color3["bc"]) + '"> — </span>' +
                    '<span style="color:' + escapeHtml(color4["fc"]) + ';background-color:' + escapeHtml(color4["bc"]) + '"> — </span>' +
                    '</div>' +
                    '</div>';
            }
    ...
    colorSelectDialog: function (currenColor, colorType, source) {
        ...
        $("body").append(replaceHtml(modelHTML, {
            ...
            "title": title,
            "content": `<div class='currenColor' data-source='${escapeHtml(source)}'>${alternatingColors.currentColor}：<span title='${escapeHtml(currenColor)}' style='background-color:${escapeHtml(currenColor)}'></span></div><div class='colorshowbox'></div>`,
    ...
    rangeDialog: function (value) {
        ...
        $("body").append(replaceHtml(modelHTML, {
            ...
            "content": `<input readonly="readonly" placeholder="${alternatingColors.tipSelectRange}" value="${escapeHtml(value)}"/>`,
