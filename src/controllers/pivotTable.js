    ...
    initialPivotManage: function (restore) {
        ...
        if (_this.initial) {
            ...
            $("#luckysheet-pivotTableFilter-submenu").mouseover(function () {
                ...
                $("#luckysheet-pivotTableFilter-selected span").text($(this).find(".luckysheet-cols-menuitem-content").text()).data("value", $(this).data("value"));
            ...
            $("#luckysheet-modal-dialog-config-row, #luckysheet-modal-dialog-config-column").on("click", ".luckysheet-modal-dialog-slider-config-item-icon", function (e) {
                ...
                let option = `<option value="self">${escapeHtml($item.find(".luckysheet-modal-dialog-slider-config-item-txt").data("name"))}</option>`;

                $("#luckysheet-modal-dialog-config-value .luckysheet-modal-dialog-slider-config-item").each(function (i) {
                    option += `<option value="${i}">${escapeHtml($(this).find(".luckysheet-modal-dialog-slider-config-item-txt").text())}</option>`;
                });
            ...
            $("#luckysheet-pivotTable-config-option-sumtype .luckysheet-cols-menuitem").click(function () {
                ...
                $item.attr("title", name).find(".luckysheet-modal-dialog-slider-config-item-txt").text(name);
            ...
            $("#luckysheet-pivotTable-selection-confirm").click(function () {
                ...
                if ($.trim(val).length == 0 || $.trim(val).toUpperCase() == _this.jgridCurrentPivotInput.toUpperCase()) {
                    ...
                    $("#luckysheet-dialog-pivotTable-range").text(val);
            ...
            $("#luckysheet-modal-dialog-config-filter, #luckysheet-modal-dialog-config-row, #luckysheet-modal-dialog-config-column, #luckysheet-modal-dialog-config-value").mousemove(function () {
                if (_this.movestate) {
                ...
                if (_this.movestate) {
                    ...
                    if (_this.movesave.containerid == $t.attr("id")) {
                        ...
                        itemHTML = `<div title="${escapeHtml(name)}" class="luckysheet-modal-dialog-slider-config-item" ${nameindex} ${sumtype} data-index="${escapeHtml(_this.movesave.index)}" data-name="${escapeHtml(_this.movesave.name)}"><div class="luckysheet-modal-dialog-slider-config-item-txt" ${nameindex} ${sumtype} data-index="${escapeHtml(_this.movesave.index)}" data-name="${escapeHtml(_this.movesave.name)}">${escapeHtml(name)}</div><div class="luckysheet-modal-dialog-slider-config-item-icon"><i class="fa fa-sort-desc" aria-hidden="true"></i></div></div>`;
                    }
            ...
            $("#luckysheet-modal-dialog-pivotTable-list").on("click", ".luckysheet-slider-list-item-selected", function () {
                ...
                if ($t.find("i").length == 0) {
                    ...
                    if (type == "num") {
                        itemHTML = `<div title="${escapeHtml(name)}" class="luckysheet-modal-dialog-slider-config-item" data-nameindex="0" data-sumtype="SUM" data-index="${index}" data-name="${escapeHtml(name)}"><div class="luckysheet-modal-dialog-slider-config-item-txt" data-nameindex="0" data-sumtype="SUM" data-index="${index}" data-name="${escapeHtml(name)}">Sum:${escapeHtml(name)}</div><div class="luckysheet-modal-dialog-slider-config-item-icon"><i class="fa fa-sort-desc" aria-hidden="true"></i></div></div>`;
                        $("#luckysheet-modal-dialog-config-value").append(itemHTML);
                    } else {
                        itemHTML = `<div title="${escapeHtml(name)}" class="luckysheet-modal-dialog-slider-config-item" data-index="${index}" data-name="${escapeHtml(name)}"><div class="luckysheet-modal-dialog-slider-config-item-txt" data-index="${index}" data-name="${escapeHtml(name)}">${escapeHtml(name)}</div><div class="luckysheet-modal-dialog-slider-config-item-icon"><i class="fa fa-sort-desc" aria-hidden="true"></i></div></div>`;
            ...
            if (restore && _this.filterparm != null) {
                if (_this.filterparm[i.toString()] != null) {
                    ...
                    if (itemset.rowhidden != null) {
                        dataother += `data-rowhidden='${escapeHtml(JSON.stringify(itemset.rowhidden))}'`;
                    }

                    if (itemset.selected != null) {
                        dataother += `data-selected='${escapeHtml(JSON.stringify(itemset.selected))}'`;
                    }
         
                    if (itemset.caljs != null) {
                        let caljsset = itemset.caljs;
                        if (caljsset.value != null) {
                            dataother += `data-byconditionvalue='${escapeHtml(caljsset.value)}'`;
                        }

                        if (caljsset.type != null) {
                            dataother += `data-byconditiontype='${escapeHtml(caljsset.type)}'`;
                        }

                        if (caljsset.text != null) {
                            dataother += `data-byconditiontext='${escapeHtml(caljsset.text)}'`;
                        }

                        if (caljsset.value1 != null) {
                            dataother += `data-byconditionvalue1='${escapeHtml(caljsset.value1)}'`;
                        }

                        if (caljsset.value2 != null) {
                            dataother += `data-byconditionvalue2='${escapeHtml(caljsset.value2)}'`;
                        }
        ...
        if (restore) {
            if (_this.filter != null && _this.filter.length > 0) {
                for (let i = 0; i < _this.filter.length; i++) {
                    ...
                    let itemHTML = `<div title="${escapeHtml(item.name)}" class="luckysheet-modal-dialog-slider-config-item" data-index="${item.index}" data-name="${escapeHtml(item.name)}"><div class="luckysheet-modal-dialog-slider-config-item-txt" data-index="${item.index}" data-name="${escapeHtml(item.name)}">${escapeHtml(item.name)}</div><div class="luckysheet-modal-dialog-slider-config-item-icon"><i class="fa fa-sort-desc" aria-hidden="true"></i></div></div>`;
            ...
            if (_this.row != null && _this.row.length > 0) {
                for (let i = 0; i < _this.row.length; i++) {
                    ...
                    if (item.order != null) {
                        otherset += `data-order = '${escapeHtml(item.order)}'`;
                    }

                    if (item.orderby != null) {
                        otherset += `data-orderby = '${escapeHtml(item.orderby)}'`;
                    }

                    if (item.order != null) {
                        otherset += `data-stastic = '${escapeHtml(item.stastic)}'`;
                    }

                    let itemHTML = `<div title="${escapeHtml(item.name)}" class="luckysheet-modal-dialog-slider-config-item" ${otherset} data-index="${escapeHtml(item.index)}" data-name="${escapeHtml(item.name)}"><div class="luckysheet-modal-dialog-slider-config-item-txt" ${otherset} data-index="${escapeHtml(item.index)}" data-name="${escapeHtml(item.name)}">${escapeHtml(item.name)}</div><div class="luckysheet-modal-dialog-slider-config-item-icon"><i class="fa fa-sort-desc" aria-hidden="true"></i></div></div>`;
            ...
            if (_this.values != null && _this.values.length > 0) {
                for (let i = 0; i < _this.values.length; i++) {
                    ...
                    if (item.sumtype != null) {
                        otherset += `data-sumtype = '${escapeHtml(item.sumtype)}'`;
                    }

                    if (item.nameindex != null) {
                        otherset += `data-nameindex = '${escapeHtml(item.nameindex)}'`;
                    }

                    let itemHTML = `<div title="${escapeHtml(item.name)}" class="luckysheet-modal-dialog-slider-config-item" ${otherset} data-index="${escapeHtml(item.index)}" data-name="${escapeHtml(item.name)}"><div class="luckysheet-modal-dialog-slider-config-item-txt" ${otherset} data-index="${escapeHtml(item.index)}" data-name="${escapeHtml(item.name)}">${_this.getSumTypeName(item.sumtype)}:${escapeHtml(item.name)}</div><div class="luckysheet-modal-dialog-slider-config-item-icon"><i class="fa fa-sort-desc" aria-hidden="true"></i></div></div>`;
        ...
        $("#luckysheet-dialog-pivotTable-range").text(getRangetxt(_this.pivotDataSheetIndex, _this.pivot_select_save));
        $("#luckysheet-modal-dialog-slider-pivot").show();
